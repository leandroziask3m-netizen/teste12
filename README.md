...
- Previsão para as próximas horas (usada na janela de aplicação)
- Usa a API OpenWeatherMap (plano "Free": 60 chamadas/min, 1.000.000/mês) — **requer chave de API**
- Clima também disponível diretamente nas máquinas (via coordenada GPS atual)

**Configuração obrigatória — variável de ambiente `OPENWEATHER_API_KEY`:**
1. Crie uma conta gratuita em https://openweathermap.org/api → plano "Free"
2. Gere uma chave em https://home.openweathermap.org/api_keys (pode levar de
   10 a 20 minutos pra ativar depois de criada)
3. Defina a variável de ambiente com essa chave:
   - Localmente: `export OPENWEATHER_API_KEY="sua_chave"` (Mac/Linux) ou
     `set OPENWEATHER_API_KEY=sua_chave` (cmd) ou
     `$env:OPENWEATHER_API_KEY="sua_chave"` (PowerShell) — no mesmo terminal,
     antes de rodar o uvicorn
   - No Render: aba do serviço → **Environment** → **Add Environment Variable**
     → Key: `OPENWEATHER_API_KEY`, Value: sua chave

> Trocado do Open-Meteo (anônimo, sem chave) porque o limite de requisições
> dele é por IP — e tanto o IP compartilhado do Render quanto o IP
> residencial/CGNAT local estavam recebendo 429 (Too Many Requests). Com
> chave de API a cota é por conta, não por IP.
