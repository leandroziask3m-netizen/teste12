...
# Cache em memória do clima por coordenada — evita gastar a cota da API a
# cada clique em "Recomendação". Chave: (lat, lng) arredondados a 3 casas (~110m).
CLIMA_CACHE = {}
CLIMA_CACHE_TTL = 600  # 10 minutos — tempo em que o dado é considerado "fresco"


def _chave_clima_cache(lat: float, lng: float):
    return (round(lat, 3), round(lng, 3))


# ---------------------------------------------------------------------------
# Provedor de clima — OpenWeatherMap (autenticado por chave de API)
# ---------------------------------------------------------------------------
#
# Trocado do Open-Meteo (anônimo, sem chave) para o OpenWeatherMap porque o
# limite de requisições do Open-Meteo é aplicado por IP — e tanto o IP
# compartilhado do Render quanto o IP residencial/CGNAT local estavam
# recebendo 429 (Too Many Requests) mesmo com poucas chamadas. Com uma chave
# de API, a cota passa a ser por CONTA, não por IP, o que resolve o problema
# de forma definitiva (plano "Free" do OpenWeatherMap: 60 chamadas/min,
# 1.000.000/mês — bem acima do que este projeto usa).
#
# Configuração necessária (faça as duas antes de testar):
#   1. Crie uma conta gratuita em https://openweathermap.org/api → plano "Free"
#   2. Gere uma chave em https://home.openweathermap.org/api_keys
#      (pode levar de 10 a 20 minutos pra "ativar" depois de criada — se der
#      HTTP 401 logo após gerar a chave, é só esperar um pouco e tentar de novo)
#   3. Defina a variável de ambiente OPENWEATHER_API_KEY com essa chave:
#        - Localmente (cmd do Windows):    set OPENWEATHER_API_KEY=sua_chave_aqui
#        - Localmente (PowerShell):        $env:OPENWEATHER_API_KEY="sua_chave_aqui"
#        - Localmente (Mac/Linux):         export OPENWEATHER_API_KEY="sua_chave_aqui"
#          (rode esse comando no MESMO terminal, antes do uvicorn)
#        - No Render: aba do serviço → "Environment" → "Add Environment Variable"
#          → Key: OPENWEATHER_API_KEY, Value: sua_chave_aqui → salvar (redeploya sozinho)
...
def buscar_clima_atual(lat: float, lng: float):
    """Busca clima atual + previsão de chuva nas próximas horas via OpenWeatherMap.

    Estratégia de resiliência:
      1. Cache em memória por coordenada por CLIMA_CACHE_TTL segundos — várias
         chamadas seguidas (cliques rápidos, várias áreas próximas) reusam o
         mesmo dado em vez de gerar uma requisição nova a cada vez.
      2. Se a API responder 429, faz até 2 tentativas extras com backoff
         (espera crescente) antes de desistir.
      3. Se mesmo assim falhar, devolve o último clima conhecido em cache
         (mesmo vencido) com a flag "desatualizado" — preferível a quebrar a
         recomendação inteira por uma instabilidade momentânea da API externa.
    """
    chave = _chave_clima_cache(lat, lng)
    agora = time.time()
    cache_entry = CLIMA_CACHE.get(chave)

    if cache_entry and (agora - cache_entry["timestamp"]) < CLIMA_CACHE_TTL:
        return cache_entry["atual"], cache_entry["diario"]

    if not OPENWEATHER_API_KEY:
        print("[clima] OPENWEATHER_API_KEY não configurada — defina essa variável de ambiente (veja instruções no código, função buscar_clima_atual)")
        if cache_entry:
            diario_fallback = dict(cache_entry["diario"] or {})
            diario_fallback["desatualizado"] = True
            return cache_entry["atual"], diario_fallback
        return None, None

    url_atual = (
        "https://api.openweathermap.org/data/2.5/weather"
        f"?lat={lat}&lon={lng}&appid={OPENWEATHER_API_KEY}&units=metric&lang=pt_br"
    )
    url_previsao = (
        "https://api.openweathermap.org/data/2.5/forecast"
        f"?lat={lat}&lon={lng}&appid={OPENWEATHER_API_KEY}&units=metric&lang=pt_br&cnt=2"
    )
    ...
    for tentativa in range(1, MAX_TENTATIVAS + 1):
        try:
            data_atual = _http_get_json(url_atual)
            try:
                data_previsao = _http_get_json(url_previsao)
            except Exception as e:
                # previsão de chuva é complementar — se falhar, segue só com o clima atual
                print(f"[clima] aviso: não foi possível obter previsão de chuva para ({lat}, {lng}): {type(e).__name__}: {e}")
                data_previsao = None
            break
        except urllib.error.HTTPError as e:
            if e.code == 401:
                print(f"[clima] HTTP 401 — OPENWEATHER_API_KEY ausente, incorreta, ou ainda não ativada (aguarde alguns minutos após criar a chave)")
                break
            if e.code == 429 and tentativa < MAX_TENTATIVAS:
                espera = 2 * tentativa  # 2s, depois 4s
                print(f"[clima] 429 Too Many Requests para ({lat}, {lng}) — tentativa {tentativa}/{MAX_TENTATIVAS}, aguardando {espera}s")
                time.sleep(espera)
                continue
            print(f"[clima] falha ao buscar clima para ({lat}, {lng}): HTTPError {e.code}: {e}")
            break
        except Exception as e:
    ...
    if data_atual is None:
        # Esgotou as tentativas (ou erro não recuperável). Usa cache vencido
        # como fallback, se existir, em vez de falhar a recomendação inteira.
        if cache_entry:
            idade_min = int((agora - cache_entry["timestamp"]) / 60)
            print(f"[clima] usando cache de {idade_min}min atrás como fallback para ({lat}, {lng})")
            diario_fallback = dict(cache_entry["diario"] or {})
            diario_fallback["desatualizado"] = True
            diario_fallback["idade_minutos"] = idade_min
            return cache_entry["atual"], diario_fallback
        return None, None

    main = data_atual.get("main", {})
    wind = data_atual.get("wind", {})
    rain = data_atual.get("rain", {})
    velocidade_vento_ms = wind.get("speed")

    atual = {
        "temperature_2m": main.get("temp"),
        "relative_humidity_2m": main.get("humidity"),
        "wind_speed_10m": round(velocidade_vento_ms * 3.6, 1) if velocidade_vento_ms is not None else None,
        "precipitation": rain.get("1h", 0.0),
    }

    chuva_proxima = None
    if data_previsao and data_previsao.get("list"):
        chuva_proxima = 0.0
        for item in data_previsao["list"][:2]:  # próximos ~6h (2 blocos de 3h)
            chuva_proxima += item.get("rain", {}).get("3h", 0.0)
            chuva_proxima += item.get("snow", {}).get("3h", 0.0)
        chuva_proxima = round(chuva_proxima, 1)

    diario = {"precipitation_proxima": chuva_proxima, "desatualizado": False}
    CLIMA_CACHE[chave] = {"timestamp": agora, "atual": atual, "diario": diario}

    return atual, diario


# ---------------------------------------------------------------------------
# Atualização de clima em segundo plano (pré-aquecimento do cache)
# ---------------------------------------------------------------------------
#
# Em vez de depender só do clique do usuário, uma thread separada tenta atualizar
# o clima das áreas que usam a recomendação agronômica (data_plantio
# preenchida) em intervalos regulares, em segundo plano. Assim, na maioria
# das vezes o cache já está aquecido quando o usuário abre a recomendação —
# em vez de a única tentativa real ser feita no exato instante do clique.

_clima_bg_thread_iniciada = False


def _atualizar_clima_areas_agronomicas():
    """Busca (e portanto atualiza o cache de) o clima de cada área que tem
    data_plantio preenchida, espaçando as chamadas pra não gerar rajada."""
    areas = load_areas()
    areas_relevantes = [a for a in areas if a.get("data_plantio")]
    for area in areas_relevantes:
        try:
            buscar_clima_atual(area["lat"], area["lng"])
        except Exception as e:
            print(f"[clima-bg] erro ao atualizar área {area.get('id')}: {type(e).__name__}: {e}")
        time.sleep(5)  # espaça as chamadas entre áreas pra não gerar rajada


def _loop_atualizacao_clima_em_segundo_plano():
    while True:
        try:
            _atualizar_clima_areas_agronomicas()
        except Exception as e:
            print(f"[clima-bg] erro no ciclo de atualização: {type(e).__name__}: {e}")
        # tenta de novo um pouco antes do cache vencer, pra manter sempre
        # um dado relativamente fresco disponível
        time.sleep(max(60, CLIMA_CACHE_TTL - 60))


@app.on_event("startup")
def iniciar_atualizacao_clima_em_segundo_plano():
    global _clima_bg_thread_iniciada
    if _clima_bg_thread_iniciada:
        return
    _clima_bg_thread_iniciada = True
    thread = threading.Thread(target=_loop_atualizacao_clima_em_segundo_plano, daemon=True)
    thread.start()
...
def recomendacao_area(area_id: int, usuario: str = Depends(get_current_user)):
    ...
    Combina:
      ...
      - Condições climáticas atuais (OpenWeatherMap): vento, chuva, umidade, temperatura
    ...
    if clima_atual is None:
        raise HTTPException(
            status_code=503,
            detail="não foi possível obter dados climáticos agora (serviço externo indisponível, limitando requisições, ou OPENWEATHER_API_KEY não configurada) e não há nenhum dado em cache para esta área ainda. Confira os logs do servidor para o motivo exato, ou tente novamente em 1-2 minutos."
        )
    ...
    return {
        ...
        "clima": {
            ...
            "dados_desatualizados": clima_diario.get("desatualizado", False) if clima_diario else False,
            "clima_atualizado_ha_minutos": clima_diario.get("idade_minutos") if clima_diario else None,
        },
