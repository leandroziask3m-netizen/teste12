...
# Cache em memória do clima por coordenada — evita bater na Open-Meteo a cada
# clique em "Recomendação". Chave: (lat, lng) arredondados a 3 casas (~110m).
CLIMA_CACHE = {}
...
def buscar_clima_atual(lat: float, lng: float):
    ...
    Estratégia anti-429 (Too Many Requests):
      ...
      2. Se a Open-Meteo responder 429, faz até 2 tentativas extras com
         backoff (espera crescente) antes de desistir.
...
def recomendacao_area(area_id: int, usuario: str = Depends(get_current_user)):
    ...
    if clima_atual is None:
        raise HTTPException(
            status_code=503,
            detail="o serviço de clima (Open-Meteo) está limitando as requisições agora (429) e não há nenhum dado em cache para esta área ainda. Tente novamente em 1-2 minutos — chamadas seguintes serão mais resilientes a esse tipo de bloqueio."
        )
